...
> **🔍 Quick Setup Below | Detailed Guide:** For comprehensive Docker workflows, docker-compose usage, Makefile shortcuts, and platform-specific troubleshooting, see **`README_docker.md`**.

### 🚀 **Quick Docker Start** (Recommended)

Choose your platform:

#### **Apple Silicon (M1/M2/M3 Macs) - Optimized Build** ⚡
```bash
# Fast ARM64-optimized build (~8-70 seconds)
docker build -f Dockerfile.arm64 -t ocr-pipeline-arm64:latest .

# Run interactively
docker run --rm -it -v "$PWD":/workspace -w /workspace ocr-pipeline-arm64:latest

# Process files directly
docker run --rm -v "$PWD/data":/app/data ocr-pipeline-arm64:latest python run_pipeline.py
```
