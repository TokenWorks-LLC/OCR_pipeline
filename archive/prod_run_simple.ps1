...
$drivePaths = @(
    "G:\.shortcut-targets-by-id\1VhQSilnVXpZtOWwAtGF6_s1zhfBIlLWU\Secondary Sources",
...
foreach ($path in $drivePaths) {
    if (Test-Path $path) {
        $sourceDir = $path
        Write-Host "[OK] Found: $sourceDir" -ForegroundColor Green
        break
    }
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Manifest build failed!" -ForegroundColor Red
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Pass A failed!" -ForegroundColor Red
    exit 1
}

Write-Host "[OK] Pass A complete!" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "[OK] Client CSV: $clientCsvA" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[WARNING] Pass B failed (non-critical, Pass A output still valid)" -ForegroundColor Yellow
} else {
    Write-Host "[OK] Pass B complete!" -ForegroundColor Green
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Final Client CSV: $clientCsvB" -ForegroundColor Green
...
---
Generated by: prod_run_simple.ps1
"@

Set-Content -Path $summaryPath -Value $summaryContent -Encoding UTF8
Write-Host "[OK] Summary written: $summaryPath" -ForegroundColor Green
