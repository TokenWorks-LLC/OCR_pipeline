#!/usr/bin/env python3
"""
Simplified Docker Gold Standard Evaluation Script

This script uses existing configurations and runs the pipeline via Docker.
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('simple_docker_evaluation.log'),
...
def run_simple_docker_pipeline(pdf_name, page_num, output_dir):
    """Run a simple Docker command to process a single PDF page"""
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        # Use the basic config as a template
        base_config_path = "config_basic.json"
        temp_config_path = os.path.join(page_output_dir, "temp_config.json")
        
        # Load and modify the basic config
        with open(base_config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
        
        # Modify for our specific task
        config['input']['process_all_files'] = False
        config['input']['specific_file'] = pdf_name
        config['output']['output_directory'] = f"./data/simple_docker_evaluation/{Path(pdf_name).stem}_page_{page_num}"
        config['processing']['specific_pages'] = [page_num]
        ...
        # Run Docker with simplified approach
        config_path_in_docker = f"data/simple_docker_evaluation/{Path(pdf_name).stem}_page_{page_num}/temp_config.json"
        docker_cmd = [
            'docker', 'compose', 'run', '--rm', 'ocr',
            'python', 'run_pipeline.py', '--config', config_path_in_docker
        ]
        
        logger.info(f"Running Docker command for {pdf_name} page {page_num}")
        
        # Use proper subprocess call without shell=True to avoid argument parsing issues
        ...
        logger.info(f"Docker pipeline completed successfully for {pdf_name} page {page_num}")
        
        # Look for the output CSV
        expected_csv = f"data/simple_docker_evaluation/{Path(pdf_name).stem}_page_{page_num}/comprehensive_results.csv"
        
        if os.path.exists(expected_csv):
            # Read the CSV and extract text
            try:
                df = pd.read_csv(expected_csv, encoding='utf-8')
                ...
                if 'corrected_text' in df.columns:
                    ...
                    logger.error(f"No text column found in {expected_csv}")
                    return None
                
                combined_text = ' '.join(str(t) for t in texts if str(t).strip())
                logger.info(f"Extracted {len(combined_text)} characters from {pdf_name} page {page_num}")
                return combined_text
                
            except Exception as e:
                logger.error(f"Error reading CSV {expected_csv}: {e}")
                return None
        else:
            logger.error(f"Expected CSV file not found: {expected_csv}")
            return None
...
def generate_report(results, output_dir):
    ...
    print("\\n" + "="*60)
    print("OCR PIPELINE GOLD STANDARD EVALUATION SUMMARY")
    print("="*60)
...
def main():
    ...
    logger.info("Starting simplified Docker-based gold standard evaluation")
    
    # Configuration
    gold_data_path = "data/gold_data/gold_pages.csv"
    input_pdfs_dir = "data/input_pdfs"
    output_dir = "data/simple_docker_evaluation"
    ...
    for i, task in enumerate(tasks, 1):
        ...
        ocr_text = run_simple_docker_pipeline(task['pdf_name'], task['page'], output_dir)
        ...
        result = {
            ...
            'page': task['page'],
            'gold_text_preview': task['gold_text'][:150] + "..." if len(task['gold_text']) > 150 else task['gold_text'],
            'ocr_text_preview': ocr_text[:150] + "..." if len(ocr_text) > 150 else ocr_text,
            **metrics
        }
    ...
    logger.info("🎉 Docker evaluation completed successfully!")
