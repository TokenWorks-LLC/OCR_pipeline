...
class MetricsCalculator:
    ...
    def calculate_cer(reference: str, hypothesis: str) -> float:
        ...
        if LEVENSHTEIN_AVAILABLE:
            ...
            distance = MetricsCalculator._levenshtein_distance(ref_norm, hyp_norm)
    ...
    def calculate_wer(reference: str, hypothesis: str) -> float:
        ...
        ref_words = MetricsCalculator.normalize_text(reference).split()
        hyp_words = MetricsCalculator.normalize_text(hypothesis).split()
        
        if LEVENSHTEIN_AVAILABLE:
            distance = Levenshtein.distance(ref_words, hyp_words)
        else:
            distance = MetricsCalculator._levenshtein_distance(ref_words, hyp_words)
    ...
    def _levenshtein_distance(s1, s2):
        ...
        if len(s1) < len(s2):
            return MetricsCalculator._levenshtein_distance(s2, s1)
